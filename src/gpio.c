...
int main(int argc, char **argv)
{
	...
	if ((argc == 3) && !strcmp(argv[1], "get")) {
		printf("get pin %s input %d\n", argv[2], mt76x8_gpio_get_pin(atoi(argv[2])));

	} else if ((argc == 4) && !strcmp(argv[1], "set")) {
		mt76x8_gpio_set_pin_direction(atoi(argv[2]), 1);
		mt76x8_gpio_set_pin_value(atoi(argv[2]), atoi(argv[3]));

	} else {
		fprintf(stderr, "%s set <pin> <value>\n", *argv);
		fprintf(stderr, "%s get <pin\n", *argv);
	}
